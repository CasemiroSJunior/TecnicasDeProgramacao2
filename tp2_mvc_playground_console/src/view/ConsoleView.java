...
public class ConsoleView {
    ...
    private iContatoRepository createRepository(String params) throws SQLException {
        // TipoBanco tipo  = TipoBanco.getTipo(params);
        /*switch (tipo) {
            case MYSQL:
            case MARIADB:
                Connection connection = ConnectionFactory.getConnection(tipo);
                ContatoDAO dao = new ContatoDAO(connection);
                return new ContatoMySqlRepositoryImpl(dao);
            case MONGODB:
                MongoClient mc = MongoClients.create("mongodb://localhost:27017");
                mc.getDatabase("fatec");
        }*/
